    ...
    def execute(self, context):
        ...
        for f in onlyfiles:
            ...
            with open('{}{}'.format(mypath,f)) as json_file:
                ...
                dfs =dfs.replace(r'\n',' ', regex=True)
                dfs['safe_title']=dfs['safe_title'].apply(lambda val: unicodedata.normalize('NFKD', val).encode('ascii', 'ignore').decode())
                dfs['transcript']=dfs['transcript'].apply(lambda val: unicodedata.normalize('NFKD', val).encode('ascii', 'ignore').decode())
                dfs['alt']=dfs['alt'].apply(lambda val: unicodedata.normalize('NFKD', val).encode('ascii', 'ignore').decode())
                dfs['title']=dfs['title'].apply(lambda val: unicodedata.normalize('NFKD', val).encode('ascii', 'ignore').decode()) 
