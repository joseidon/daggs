    ...
    def execute(self, context):
        ...
        for f in onlyfiles:
            ...
            with open('{}{}'.format(mypath,f)) as json_file:
                ...
                dfs =dfs.replace(r'\n',' ', regex=True) 
                dfs["safe_title"] =dfs["safe_title"].encode("ascii", "replace")
                dfs["transcript"] =dfs["transcript"].encode("ascii", "replace")
                dfs["alt"] =dfs["alt"].encode("ascii", "replace")
                dfs["title"] =dfs["title"].encode("ascii", "replace")
                #print(dfs)
                data.append(dfs)
        print("Print List:")
